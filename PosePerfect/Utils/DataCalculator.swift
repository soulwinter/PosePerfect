...
// 计算两个四元数之间的角度差异
func quaternionAngleDifference(_ q1: CMQuaternion, _ q2: CMQuaternion) -> Double {
    // 计算点积
...
// 计算两个四元数之间的余弦相似度
func cosineSimilarityBetween(_ quaternion1: CMQuaternion, _ quaternion2: CMQuaternion) -> Double {
    // 计算四元数向量的点积
...
func quaternionBetween(_ quaternion1: CMQuaternion, _ quaternion2: CMQuaternion) -> CMQuaternion {
    // 计算 quaternion1 的逆
    let inverseQuaternion1 = CMQuaternion(x: -quaternion1.x, y: -quaternion1.y, z: -quaternion1.z, w: quaternion1.w)

    // 计算两个四元数之间的旋转
    let rotationQuaternion = CMQuaternion(
...
func quaternionToEulerAngles(quaternion: CMQuaternion) -> (Double, Double, Double) {
    let qw = quaternion.w
    let qx = quaternion.x
    let qy = quaternion.y
    let qz = quaternion.z
...
// 计算3D角的差异
func differenceAcceleration(vector1: CMAcceleration, vector2: CMAcceleration) -> Double {
